...
/**
 * Fetches NFT metadata assets for a given contract address from the MetaPro Protocol NFT service.
 *
 * This function makes an asynchronous HTTP GET request to the MetaPro Protocol API, retrieving
 * a list of NFTs associated with a specific contract address. The NFTs are sorted by their
 * creation block in descending order. The response is returned as a parsed JSON object.
 *
 * @param {string} contractAddress - The contract address for which NFT metadata assets are being fetched.
 *
 * @returns {Promise<any>} - A promise that resolves to the list of NFT metadata assets associated
 ...
 *
 * fetchNftByContractAddress(contractAddress)
 *   .then((nftAssets) => {
 *     console.log('NFT Assets:', nftAssets);
 *   })
 *   .catch((error) => {
